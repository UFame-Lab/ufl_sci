...
class SCI_node_test_rDuuDLwtLOuBbkrJoosemMLZcMPXqsknWjpSHriZrR(unittest.TestCase):
    """
    - `"sci_mode": "shadow"`
    ...
    - `saturn` -(remote-rq)-> `appolo (wsBridgeBroker)` -(ws)-> `jupiter` 
    """
    def setUp(self):
        SCI_SETTINGS_appolo = {
        ...
        SCI_SETTINGS_shadow = {
            "related_node_rq": "sci_rq_saturn_node_aCtn4R2k3qPtziyBEM6EwjdDr7ulRz5WQHDbOZ",
            "logfilePath": TEST_LOG_FILE_PATH,
            "local_broker_connection_settings": {
                "redis": {
                    "host": {
    ...
    def test_YRCJHkikvNOx(self):
        ...
        Особенности:
        - Узел получатель - 
        `sci_node_saturn` отправляет сообщение в `sci_node_jupiter`.
    ...
    def test_lSnsXzTXmpJkTTmE(self):
        ...
        Особенности:
        - Узел получатель - 
        `sci_node_saturn` отправляет сообщение в неизвестный `SCI` node
        - Приложение получатель зарегистрировано в `SCI_SETTINGS -> "AppConf"`
        - `EventMessage -> "action"` передан поддерживаемым `Application`
        - `ping-pong` отправляется.
...
class SCI_node_test_TWzrAFIPIqrhbkbljDfKbwcIwuKScGJxynuaBJ(unittest.TestCase):
    """
    - `"sci_mode": "shadow"`
    - `without await response`
    - `"interface": "ws"`
    - `use wsBridgeBroker via remote-rq interface`
    - `EventMessage` `proxy` отправляется от:
    - `jupiter` -(ws)-> `appolo (wsBridgeBroker)` -(remote-rq)-> `saturn` 
    """
    def setUp(self):
        SCI_SETTINGS_appolo = {
            "node_name": "appolo_node_aCtn4R2k3qPtziyBEM6EwjdDr7ulRz5WQHDbOZ",
    ...
    def test_YRCJHkikvNOx(self):
        ...
        Особенности:
        - Узел получатель - 
        `sci_node_jupiter` отправляет сообщение в `sci_node_saturn`.
        - Приложение получатель зарегистрировано в `SCI_SETTINGS -> "AppConf"`
        - `EventMessage -> "action"` передан поддерживаемым `Application`
        - `ping-pong` не отправляется.
       ...
       - Узел получатель - 
        ...
        async def send_message(ping_pong: bool = False):
            ...
            EventMessage = {
                ...
                "action": "test-generic",
                "address_section": {
                    "recipient": "saturn_node_aCtn4R2k3qPtziyBEM6EwjdDr7ulRz5WQHDbOZ:wrong_test_app",
                    "interface": "ws",
                    "wsbridge": "alfaBridge",
                },
                "meta": {
