...
class SCI_node_test_lEFuAreCGOViYDZZmuUHWNzsnvRBscTaNyroMwxoNbLv(unittest.TestCase):
    """
    - `"sci_mode": "standart"`
    - `with await response`
    - `"interface": "ws"`
    - `EventMessage` отправляется от `jupiter` к `appolo`
    - `EventMessage` отправляется от `appolo` к `jupiter` 
    """
    def setUp(self):
        SCI_SETTINGS_appolo = {
    ...
    def test_FLOaNuAkAYGadybDUJcfPuTxDslEvzjymXTtUzWCnSV(self):
        ...
        Особенности:
        - Узел получатель - `sci_node_jupiter` отрправляет сообщение в неизвестный
        узел.
        - Приложение получатель зарегистрировано в `SCI_SETTINGS -> "AppConf"`
        - `EventMessage -> "action"` передан поддерживаемый целевым `Application`
        - `ping-pong` используется.
