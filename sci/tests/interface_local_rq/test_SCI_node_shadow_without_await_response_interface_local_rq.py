...
class SCI_node_test_qaVkHnSAXonhrpXMHHLdrnxPpDFzNvrgKhoV(unittest.TestCase):
    """
    - `"sci_mode": "shadow"`
...
class SCI_node_test_zZCfPXSquFSmbjLunKoUsRZfdOPWJPrlaUDnpEeM(unittest.TestCase):
    """
    - `"sci_mode": "shadow"`
    - `without await response`
    - `"interface": "local-rq"`
    ...
    def test_lSnsXzTXmpJkTTmE(self):
        ...
        Особенности:
        - Узел получатель - 
        `sci_node_fn` отправляет сообщение в неизвестный `SCI` node
        - Приложение получатель зарегистрировано в `SCI_SETTINGS -> "AppConf"`
        - `EventMessage -> "action"` передан поддерживаемым `Application`
        - `ping-pong` отправляется.
