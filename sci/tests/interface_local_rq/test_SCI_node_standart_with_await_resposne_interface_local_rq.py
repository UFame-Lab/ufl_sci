...
class SCI_node_test_SBGZmHdJwLnKJCEbhLaRTkEFHoxkTzxjEh(unittest.TestCase):
    """
    - `"sci_mode": "standart"`
    - `with await response`
    - `"interface": "local-rq"`
    ...
    def test_kImkUBfrfXvNLDWhCYdrhhnexRlaHcOSftgQqYGNZAD(self):
        ...
        async def send_message(ping_pong: bool = False) -> dict:
            ...
            send_result = await self.sci_cli_jupiter.send_message(
                EventMessage_from_sn, ping_pong=ping_pong
            )
            return send_result
        # Запуск сперва `sci_core`, затем `*send_message`
        result_from_sn_to_fn = asyncio.run(
            self.async_test_runner([send_message(ping_pong=False)])
        )
        self.assertEqual(True, isecsaddo(result_from_sn_to_fn), msg=msg)
        self.assertEqual("ok", result_from_sn_to_fn["status"], msg=msg)
        self.assertIn("awaitable_response", result_from_sn_to_fn["data"], msg=msg)
        self.assertEqual(
            True, 
            inspect.isfunction(result_from_sn_to_fn["data"]["awaitable_response"]),
            msg=msg
        )
    ...
    def test_nTJhixegGuRljqQVrEtOMyTRSsdbpOuTGBGfpSdqqNUp(self):
        ...
        Особенности:
        - Сообщение передается от `appolo` узла, в `jupiter` узел.
        - Приложение получатель зарегистрировано в `SCI_SETTINGS -> "AppConf"`
        - `EventMessage -> "action"` передан поддерживаемый целевым `Application`
        - `ping-pong` не используется
        ...
        async def send_message(ping_pong: bool = False) -> dict:
            ...
            send_result = await self.sci_cli_appolo.send_message(
                EventMessage_from_fn, ping_pong=ping_pong
            )
            return send_result
        # Запуск сперва `sci_core`, затем `*send_message`
        result_from_fn_to_sn = asyncio.run(
            self.async_test_runner([send_message(ping_pong=False)])
        )
        self.assertEqual(True, isecsaddo(result_from_fn_to_sn), msg=msg)
        self.assertEqual("ok", result_from_fn_to_sn["status"], msg=msg)
        self.assertIn("awaitable_response", result_from_fn_to_sn["data"], msg=msg)
        self.assertEqual(
            True, 
            inspect.isfunction(result_from_fn_to_sn["data"]["awaitable_response"]),
            msg=msg
        )
    ...
    def test_UtPwYATxU(self):
        ...
        Особенности:
        - Узел получатель - `sci_node_appolo` отрправляет сообщение в неизвестный
        узел.
        - Приложение получатель зарегистрировано в `SCI_SETTINGS -> "AppConf"`
        - `EventMessage -> "action"` передан поддерживаемый целевым `Application`
    ...
    def test_dqsvhaiclZuEsoEHylNHfbbYFUBZfKYCtkc(self):
        ...
        async def send_message(ping_pong: bool = False):
            ...
            send_first_result = await self.sci_cli_appolo.send_message(
                EventMessage_first,
                ping_pong=ping_pong
            )
            await asyncio.sleep(0.1)
            send_second_result = await self.sci_cli_appolo.send_message(
                EventMessage_second,
                ping_pong=ping_pong
            )
            return send_second_result
    ...
    def test_RXrSOJbfvzZGbKsxUsJpBCJqOnRYQDiOvwL(self):
        """
        Основные условия:
        - `"sci_mode" - "standart"`
        - `"mType": "request"` 
        ...
        - `response.session_result` - `ecsaddo` "ok" `response` сессии. 
        """
        msg = self.test_RXrSOJbfvzZGbKsxUsJpBCJqOnRYQDiOvwL.__doc__
        async def send_message(ping_pong: bool = False):
            await asyncio.sleep(WAIT_AUTH)
            EventMessage = {
                "sender": "anonim",
    ...
    def test_FxCcfZRqHGefnEYaSgBZUjPyeovtWdeL(self):
        """
        Основные условия:
        - `"sci_mode" - "standart"`
        - `"mType": "request"` 
        ...
        - `response.session_result` - `ecsaddo` "ok" `response` сессии. 
        """
        msg = self.test_FxCcfZRqHGefnEYaSgBZUjPyeovtWdeL.__doc__
        async def send_message(ping_pong: bool = False):
            await asyncio.sleep(WAIT_AUTH)
            error_ecsaddo = create_ecsaddo(
                "error",
