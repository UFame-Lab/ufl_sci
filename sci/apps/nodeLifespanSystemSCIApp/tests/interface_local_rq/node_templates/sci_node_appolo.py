...
async def start_sci_node():
    try:
        ...
        sci_node_appolo = SCI(sci_mode="standart", SCI_SETTINGS=SCI_SETTINGS_appolo)
        sci_cli_appolo, sci_core_appolo = sci_node_appolo.start()
        task_sci_core_appolo = asyncio.create_task(sci_core_appolo())
        while True:
            if not sci_core_appolo.is_working:
                await asyncio.sleep(0.1)
                continue
            break
        await task_sci_core_appolo
    except Exception as ex:
        trc = str(traceback.format_exception(ex))
        print(trc)
        for task in asyncio.all_tasks(loop):
            if not task.done() and task.get_name() != current_task_name:
                task.cancel()
        redis_conn_appolo = (
            sci_core_appolo.SCI_SETTINGS.get(
                "local_broker_connection_settings"
            )["redis"]["redis_conn"]
        )
