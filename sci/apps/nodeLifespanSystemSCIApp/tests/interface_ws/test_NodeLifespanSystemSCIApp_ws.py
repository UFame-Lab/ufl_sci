...
class NodeLifespanSystemSCIApp_YXddLPmtgGhelKGHGok(unittest.TestCase):
    """
    ...
    def test_MRnMzqrHZbqXFbNqIrJxuArgGzWhsqndiTsY(self):
        ...
        self.assertIn(
            ...
            SCI_SETTINGS_appolo["nodes"]["remote_ws_nodes"], 
            msg=msg
        )        
        self.assertEqual(
            "alfaBridge", 
            SCI_SETTINGS_appolo["nodes"]["remote_ws_nodes"].get(
                "jupiter_node_aCtn4R2k3qPtziyBEM6EwjdDr7ulRz5WQHDbOZ"
            )["wsbridge"],
            msg=msg
        )
    ...
    def test_nRIedrgrpfZqOXEfoRJaoBJTQtgaxniavPUJISICM(self):
        ...
        self.assertEqual(
            True,             
            (time_end_test - SCI_SETTINGS_appolo["nodes"]["remote_ws_nodes"].get(
                "jupiter_node_aCtn4R2k3qPtziyBEM6EwjdDr7ulRz5WQHDbOZ"
    ...
    def test_lokwFusMzaaWpRiUxXsaqKrBrvrRZkVNmfRiQBfuGH(self):
        ...
        Проверка:
        `SCI_SETTINGS -> "nodes" -> "remote_ws_nodes"` должен быть пустым.
        """
        ...
        # SCI_SETTINGS_fn = self.sci_cli_fn.SCI_SETTINGS
        SCI_SETTINGS_jupiter = self.sci_cli_jupiter.SCI_SETTINGS
        self.assertEqual(
            0,
...
class NodeLifespanSystemSCIApp_WsROvvbTuQzqRIEK(unittest.TestCase):
    """
    Тесты нормальной работы аутентификации NodeLifespanSystemSCIApp
    - `"interface": "ws"`
...
class NodeLifespanSystemSCIApp_JkobNnLhPcEfbUlTdaWpPxqtqea(unittest.TestCase):
    """
    Тесты нормальной работы аутентификации NodeLifespanSystemSCIApp
    - `"interface": "ws"`
...
class NodeLifespanSystemSCIApp_RkgMtbSsEIrwZorsTOrHWlo(unittest.TestCase):
    """
    Тесты нормальной работы аутентификации NodeLifespanSystemSCIApp
    - `"interface": "ws"`
    ...
    def start_sci_node_process(self) -> subprocess.Popen:
        path_to_module = str(
            str(Path(__file__).resolve().parent) + 
            "/node_templates/sci_node_jupiter_auth_appolo_use_ws.py"
        )
        process = subprocess.Popen(["python3.11", path_to_module])
        self.background_processes.add(process)
        return process
    ...
    def test_vGEDhVJXEtUjkQAaWOcSdwUEYvQDna(self):
        ...
        self.assertEqual(
            0,
            len(SCI_SETTINGS_appolo["nodes"]["remote_ws_nodes"]),
            msg=msg
        )
        
        
class NodeLifespanSystemSCIApp_gFTZkooMKKUvOvNglV(unittest.TestCase):
    """
    Тесты нормальной работы аутентификации NodeLifespanSystemSCIApp
    - `"interface": "ws"`
    ...
    def test_vGEDhVJXEtUjkQAaWOcSdwUEYvQDna(self):
        ...
        `sci_node_jupiter` в `SCI_SETTINGS -> "nodes" -> "remote_ws_nodes"`.
