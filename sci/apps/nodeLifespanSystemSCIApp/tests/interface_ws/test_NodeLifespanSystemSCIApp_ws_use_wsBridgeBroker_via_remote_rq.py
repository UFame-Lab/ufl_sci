...
class NodeLifespanSystemSCIApp_rtBbuTWviniQHPaYLaRRKLbmMhCYCVxkJcvc(unittest.TestCase):
    """
    Тесты нормальной работы аутентификации NodeLifespanSystemSCIApp
    - `"interface": "ws"`
    - `use wsBridgeBroker via remote-rq interface`
    """
    ...
    def test_MRnMzqrHZbqXFbNqIrJxuArgGzWhsqndiTsY(self):
        ...
        - `sci_node_saturn` - "standart" / use wsBridgeBroker via remote-rq 
        interface
...
class NodeLifespanSystemSCIApp_RFyjkDCLbExLoYwPuNDxE(unittest.TestCase):
    """
    Тесты нормальной работы аутентификации NodeLifespanSystemSCIApp
    - `"interface": "ws"`
    - `use wsBridgeBroker via remote-rq interface`
    ...
    def test_wSyvhHEIlUCAcwXuzGFlPHDQKpjsgftLEpwgR(self):
       ...
       Основные конфигурации:
        ...
        `SCI_SETTINGS -> "nodes" -> "remote_nodes" / "remote_ws_nodes"`.
        ...
        self.assertEqual(
            0,
            len(SCI_SETTINGS_saturn["nodes"]["remote_nodes"]),
            msg=msg
        )
        self.assertEqual(
            0,
...
class NodeLifespanSystemSCIApp_lbHNiwxbKCVXJFLSpaaIsssw(unittest.TestCase):
    """
    Тесты нормальной работы аутентификации NodeLifespanSystemSCIApp
    - `"interface": "ws"`
    - `use wsBridgeBroker via remote-rq interface`
    ...
    def test_vGEDhVJXEtUjkQAaWOcSdwUEYvQDna(self):
        ...
        via remote-rq interface (В текущем процессе)
...
class NodeLifespanSystemSCIApp_OPncyWZOfpgGCDRAnRMdsbJoOywjTmsH(unittest.TestCase):
    """
   Тесты нормальной работы аутентификации NodeLifespanSystemSCIApp
    - `"interface": "ws"`
    - `use wsBridgeBroker via remote-rq interface`
...
class NodeLifespanSystemSCIApp_nJFTKGcEyMGSMsJnirJGlzWIsGNmpSa(unittest.TestCase):
    """
    Тесты нормальной работы аутентификации NodeLifespanSystemSCIApp
    - `"interface": "ws"`
    - `use wsBridgeBroker via remote-rq interface`
    ...
    def start_sci_node_process(self) -> subprocess.Popen:
        path_to_module = str(
            str(Path(__file__).resolve().parent) + 
            "/node_templates/sci_node_appolo_auth_saturn_use_remote_rq.py"
        )
        process = subprocess.Popen(["python3.11", path_to_module])
        self.background_processes.add(process)
        return process
...
class NodeLifespanSystemSCIApp_wRgumbOzJPHIMffvOreIDpn(unittest.TestCase):
    """
    Тесты нормальной работы аутентификации NodeLifespanSystemSCIApp
    - `"interface": "ws"`
    - `use wsBridgeBroker via remote-rq interface`
