...
class NodeLifespanSystemSCIApp_PGMpFEZBEKOkwqIVDW(unittest.TestCase):
    """
    ...
    def test_lokwFusMzaaWpRiUxXsaqKrBrvrRZkVNmfRiQBfuGH(self):
        ...
        Проверка:
        `SCI_SETTINGS -> "nodes" -> "remote_nodes"` должен быть пустым.
        """
        ...
        self.assertEqual(
            0,
            len(SCI_SETTINGS_jupiter["nodes"]["remote_nodes"]),
            msg=msg
        )
...
class NodeLifespanSystemSCIApp_qTXCjEIZZfYtgazInevwMdoqYv(unittest.TestCase):
    """
    Тесты нормальной работы аутентификации NodeLifespanSystemSCIApp
    - `"interface": "remote-rq"`
...
class NodeLifespanSystemSCIApp_TuRfWhQfFHXNMxAiyHLqzNBbpRdT(unittest.TestCase):
    """
    Тесты нормальной работы аутентификации NodeLifespanSystemSCIApp
    - `"interface": "remote-rq"`
...
class NodeLifespanSystemSCIApp_nUyTKhuRsGMupJnuaktj(unittest.TestCase):
    """
    Тесты нормальной работы аутентификации NodeLifespanSystemSCIApp
    - `"interface": "remote-rq"`
    ...
    def start_sci_node_process(self) -> subprocess.Popen:
        path_to_module = str(
            str(Path(__file__).resolve().parent) + 
            "/node_templates/sci_node_jupiter_auth_appolo_use_remote_rq.py"
        )
        process = subprocess.Popen(["python3.11", path_to_module])
        self.background_processes.add(process)
        return process
    ...
    def test_vGEDhVJXEtUjkQAaWOcSdwUEYvQDna(self):
        ...
        self.assertEqual(
            0,
            len(SCI_SETTINGS_appolo["nodes"]["remote_nodes"]),
            msg=msg
        )
        
        
class NodeLifespanSystemSCIApp_NNDBOAyDqUIMNIZhaDA(unittest.TestCase):
    """
    Тесты нормальной работы аутентификации NodeLifespanSystemSCIApp
    - `"interface": "remote-rq"`
    ...
    def test_vGEDhVJXEtUjkQAaWOcSdwUEYvQDna(self):
        ...
        `sci_node_jupiter` в `SCI_SETTINGS -> "nodes" -> "remote_nodes"`.
